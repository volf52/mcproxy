	...
	tests := []struct {
		...
		},
		{
			...
			endpoints: map[string]Endpoint{
				"api": {
					Url:     "https://api.example.com",
					Timeout: func() *time.Duration { d := time.Duration(-1); return &d }(),
				},
			},
			expectError: true,
		...
		},
		{
			...
			endpoints: map[string]Endpoint{
				"api": {
					Url:     "https://api.example.com",
					Timeout: func() *time.Duration { d := time.Duration(400) * time.Second; return &d }(),
				},
			},
			expectError: true,
		...
		},
		{
			...
			endpoints: map[string]Endpoint{
				"api": {
					Url:         "https://api.example.com",
					MaxBodySize: func() *int64 { s := int64(-1); return &s }(),
				},
			},
			expectError: true,
		...
		},
		{
			...
			endpoints: map[string]Endpoint{
				"api": {
					Url:         "https://api.example.com",
					MaxBodySize: func() *int64 { s := int64(200 * 1024 * 1024); return &s }(),
				},
			},
			expectError: true,
